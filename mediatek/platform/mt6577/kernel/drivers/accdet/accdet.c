...
//A switch to ouput 2.8v to earbias to trigger the IC of iphone headset.
#define GPIO_JRD_IPHONE_HDST_TRIGGER GPIO126
#define JRD_IPHONE_TRIGGER_ON GPIO_OUT_ONE
#define JRD_IPHONE_TRIGGER_OFF GPIO_OUT_ZERO
//switch earbias through a nmos
#define GPIO_JRD_IPHONE_HDST_EARBIAS_SWITCH GPIO114 
#define JRD_EARBIAS_MOS_ON GPIO_OUT_ZERO
#define JRD_EARBIAS_MOS_OFF GPIO_OUT_ONE

static int jrd_iphone_hdst_debug_on = 0;
static int jrd_iphone_hdst_delay[4] = {0, 5, 100, 5};//unit: ms

extern int jrd_get_record_stat(void);
...
void accdet_eint_work_callback(struct work_struct *work)
{
   ...
   //ACCDET_DEBUG("[Accdet]eint work callback do nothing!!!!!!\n");  
    if (cur_eint_state == EINT_PIN_PLUG_IN) {
        ...
        current_status = INREG32(ACCDET_CURR_IN) & 0x3; //A=bit1; B=bit0
...
#if 1//Added by jrd.lipeng for telsa PIO: EINT8_HP has no pull-up resistance in HW. So pull it up in SW.
	mt_set_gpio_pull_enable(GPIO_ACCDET_EINT_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_ACCDET_EINT_PIN, GPIO_PULL_UP);
#endif
...
void jrd_trigger_iphone_hdst(void)
{
    ...
    //trigger off
    mt_set_gpio_dir(GPIO_JRD_IPHONE_HDST_TRIGGER, JRD_IPHONE_TRIGGER_OFF);
}
...
#ifdef JRD_IPHONE_HDST_COMPATIBILITY
                        /* Why will this ioctl cmd be called many times every entering call mode? */
                        if(call_status == 2) {
                            jrd_trigger_iphone_hdst();
                        }
#endif
...
/* 3 use cases:
  * 1. D00:d5:10:d100:01
  * when write this string to iphone_hdst_debug, then following seriel commands will be executed:
  *     1.1 set jrd_iphone_hdst_debug_on to 1. Then jrd_trigger_iphone_hdst() becomes an empty function.
  *     1.2 msleep(0)
  *     1.3 trigger off, earbias disconnect
  *     1.4 msleep(5)
  *     1.5 trigger on, earbias disconnect
  *     1.6 msleep(100)
  *     1.7 trigger off, earbias connect
  * 2. 00:d5:10:d100:01
  * The same with above one, except no jrd_iphone_hdst_debug_on setting.
  * 3. S0:5:100:5
  *     3.1 set jrd_iphone_hdst_debug_on to 0
  *     3.2 set jrd_iphone_hdst_delay[4] which will be used in jrd_trigger_iphone_hdst().
  * */
