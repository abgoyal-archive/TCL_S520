...
#define JRD_BOOTUP_CAL//Added by jrd.xinglan for boot-up calibration.
#define XL_BOOTUP_CAL_DEBUG //Added by jrd.xinglan for boot-up calibration debug.
#if defined(XL_BOOTUP_CAL_DEBUG)//xinglan  tmp debug
static int cal_params[4] = {0};//ps max, mean, high, low
#endif
...
#if defined(JRD_BOOTUP_CAL)//Modified by jrd.xinglan for boot-up calibration.
...
#if 0
	databuf[0] = TMD2771_CMM_OFFSET;    //  ADD OFFSET REGISTER 2012.10.24
	databuf[1] = 0x40;//0xFF 
	res = i2c_master_send(client, databuf, 0x2);
	if(res <= 0)
	{
		goto EXIT_ERR;
		return TMD2771_ERR_I2C;
	}				    //  ADD OFFSET REGISTER 2012.10.24
#endif 
	/*for interrup work mode support -- by liaoxl.lenovo 12.08.2011*/
...
//	u16 ps_value;    
	...
	*data = ps_value_low[0] | (ps_value_high[0]<<8);
	...
	res = i2c_master_recv(client, sensor_id, 0x01);
	printk("TAOS_TMD277xx....sensor_id[0]=%x\n",sensor_id[0]);

#if defined(XL_BOOTUP_CAL_DEBUG)
        APS_ERR("ps_data=%d, low:%d  high:%d", *data, ps_value_low[0], ps_value_high[0]);
        APS_ERR("jrd.xinglan|calibration: max=%d, mean=%d, high=%d, low=%d\n", cal_params[0], cal_params[1], cal_params[2], cal_params[3]);
#endif
	return 0;    
...
//	 u16 temp_ps[1];
	...
	//APS_LOG("tmd2771_get_ps_value  sensor_id %d,", sensor_id);
	if(ps_cali.valid == 1)
		{
...
#if defined(JRD_BOOTUP_CAL)//Added by jrd.lipeng for boot-up calibration
/*----------------------------------------------------------------------------*/
...
static void tmd2771_ps_calibrate(struct i2c_client *client)
{
	...
	prox_threshold_hi = prox_mean*18/10;
	...
	//prox_threshold_lo = ((((prox_max - prox_mean) * 310) + 50) / 100) + prox_mean;
        ...
        #if defined(XL_BOOTUP_CAL_DEBUG)//xinglan  debug
...
#if defined(JRD_BOOTUP_CAL)//Added by jrd.xinglan for boot-up calibration
...
static int tmd2771_probe(struct platform_device *pdev) 
{
	...
	tmd2771_power(hw, 1);
        msleep(1);//Added by jrd.xinglan. Without this delay, i2c_master_send() will fail.
